...
export default function Hero({ onGetStarted }) {
  ...
  useEffect(() => {
    // 첫 번째 카피(index 0)는 9.5초(9500ms), 나머지는 7.5초(7500ms)
    const duration = currentIndex === 0 ? 9500 : 7500;
  ...
  return (
    <section className="text-center pt-14 sm:pt-[88px] pb-16 sm:pb-20 px-4 sm:px-5 min-h-[600px] flex flex-col justify-center items-center">
      ...
      <div className={`min-h-[280px] md:min-h-[320px] flex flex-col justify-center transition-opacity duration-500 ease-in-out ${fade ? 'opacity-100' : 'opacity-0'}`}>
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tighter text-slate-900 leading-tight mb-6 sm:mb-8">
        ...
        <p className="text-base sm:text-lg md:text-xl text-slate-600 max-w-2xl mx-auto mb-8 sm:mb-10 leading-relaxed font-medium whitespace-pre-line px-2">
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4 mt-2 w-full px-4 sm:px-0">
        <button 
          className="w-full sm:w-auto bg-blue-600 text-white font-semibold py-2.5 px-5 md:px-6 rounded-full text-sm md:text-base shadow-[0_10px_30px_rgba(37,99,235,0.3)] hover:-translate-y-1 hover:shadow-[0_15px_40px_rgba(37,99,235,0.4)] transition-all cursor-pointer border-none"
          onClick={onGetStarted}
        >
          무료로 시작하기
        </button>
        <button 
          className="w-full sm:w-auto bg-white text-slate-700 border border-slate-200 font-semibold py-2.5 px-5 md:px-6 rounded-full text-sm md:text-base shadow-[0_10px_30px_rgba(0,0,0,0.05)] hover:-translate-y-1 hover:shadow-[0_15px_40px_rgba(0,0,0,0.1)] transition-all cursor-pointer"
